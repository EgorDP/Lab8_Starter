describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const button = await itemShadow.$('button');
    ...
    // Make sure button text changes to 'Remove from Cart'
  ...
  // number in the top right has been correctly updated
   it('Checking number of items in cart on screen', async () => {
    ...
    // TODO - Step 3
    const prodItems = await page.$$('product-item');
    // Get shadowroot and query select button for each item 
    ...
    let cartCount_ = await  cart.getProperty('innerText');
    // Get real count of items in cart 
    let cartCount = cartCount_['_remoteObject'].value;
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 1; i < prodItems.length; ++i){
      ...
      // Once you have the innerText property, use innerText['_remoteObject'].value to get the text value of it
      let textValue = text['_remoteObject'].value;
      // Make sure button text is now 'Remove from Cart'
      expect(textValue).toBe('Remove from Cart');
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; ++i){
      ...
      let  text = await button.getProperty('innerText');
      // Once you have the innerText property, use innerText['_remoteObject'].value to get the text value of it
      let textValue = await text['_remoteObject'].value;
      // Make sure button text is now 'Remove from Cart'
